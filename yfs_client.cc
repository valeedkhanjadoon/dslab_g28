...
{
  int r = OK;
  // You modify this function for Lab 03
...
int yfs_client::createfile(inum p_inum, const char *name, inum &c_num, bool isfile)
{
  int r = OK, count = 0;
  std::string p_buf;
  char *cstr, *p;
...
int yfs_client::createroot(inum in_inum, const char *name)
{
  int r = OK;
  lc->acquire(in_inum);
...
int yfs_client::readdir(inum p_inum, std::vector<dirent> &r_dirent)
{
  int r = OK, count = 0;
  std::string p_buf, inum_buf;
  char *cstr, *p;
...
int yfs_client::unlink(inum p_inum, const char *name)
{
  int r = NOENT, count = 0;
